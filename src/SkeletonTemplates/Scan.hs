module SkeletonTemplates.Scan where
...
scanActor actorName initValue (Dimension w h) (R.TwoVarFunC pixelIdent stateIdent exp) incomingType outgoingType =
  let ioSig =
      ...
      foldActionOutPattern =
        [ C.OutPattTagIds
            (C.Ident "Out1")
            [C.OutTokenExp (C.EIdent (idRiplToCal stateIdent))]
        ]
      foldActionGuard =
        C.BrExpCons
          (C.BELT
        ...
        C.ActionCode
          (C.AnActn
             (C.ActnTagsStmts
                ...
                outputStateBody))
      priorityBlock =
        [C.PriOrd [C.PriInEQ (C.Ident "scan") (C.Ident "outputState") []]]
      globalVars =
        [ C.GlobVarDecl
            (C.VDeclExpIMut
               (uintCalType 32)
